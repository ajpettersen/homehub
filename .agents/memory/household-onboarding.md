...
- The first-login setup wizard gates on household administrators with `role === 'family'`, `isAdmin === true`, AND the household's `onboarding_completed_at` being NULL. Pending users, cleaners, and non-admin family members never see it.
...
- Both finishing and skipping first-login setup are administrator-only operations, so every client gate must include `isAdmin`; otherwise an incomplete non-admin household member is trapped in a wizard they cannot complete.

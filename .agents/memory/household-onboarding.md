...
- All wizard seeding (household rename, property, members, starter grocery list/chores/maintenance) goes through ONE transactional endpoint (`POST /onboarding`) that sets the completion flag in the same transaction and no-ops (`alreadyCompleted: true`) if the flag is already set.
...
- **How to apply:** any new onboarding surface (e.g. mobile) or new seed content should reuse/extend this endpoint rather than issuing per-entity creates from the client. Completion is one-way: the API contract only accepts `onboardingCompleted: true`.

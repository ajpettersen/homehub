...
**Administrator boundary:** Every approved, account-linked adult may create/revoke household invites and approve/deny join requests. Sensitive household configuration—including onboarding/setup, household-wide navigation, and account role changes—keeps its elevated authorization boundary.
...
**Adult member-management boundary:** Every approved account-linked adult may edit household member profiles and merge an unlinked legacy adult into a linked adult. Account-linked adults are never directly deletable, and referenced legacy records must be merged or reassigned rather than discarded.

**Why:** Family profile upkeep is shared adult responsibility, but deleting linked identities or orphaning historical assignments would corrupt household history.

**Property management boundary:** Every approved account-linked adult may add and edit properties. Deletion is allowed only for a non-final property with no household records or access assignments.

**Why:** Property upkeep is ordinary shared household work, but cascading a deletion into chores, maintenance, meals, lists, people, or cleaner access would destroy history.

**How to apply:** Keep pending accounts outside the application shell until approved or invited. Joining must create or explicitly claim one adult identity transactionally. Invite bearer tokens belong in URL fragments and request bodies only, are stored hashed, expire, and are consumed once; never place them in request paths, referrers, or logs. Gate all joining methods on the same server-backed personal-setup state. Preserve ambiguous legacy adults for explicit reconciliation. Transfer all references atomically before deleting a legacy duplicate. Check every property reference before deletion and require reassignment when any remain. Self-service edits may change presentation fields only; household identity, elevated roles, and destructive changes remain server-controlled.
...
def replace_block(text, start, end, new_block):
    ...
    if idx1 == -1 or idx2 == -1:
        return text
    return text[:idx1] + new_block + text[idx2 + len(end):]
...
new_func = """function GroceryListDetail({ list, meals }: { list: any; meals: MealForShopping[] }) {
  ...
  const mappedKeys = new Set(orderedKeys);
...
if idx1 != -1:
    idx2 = content.find("}\n", content.rfind("    </div>\n  );\n}"))
    if idx2 != -1:
        new_c = content[:idx1] + new_func + content[idx2+2:]
        with open("artifacts/home-hub-web/src/pages/Kitchen.tsx", "w") as f:
            f.write(new_c)
        print("Kitchen.tsx replaced correctly.")

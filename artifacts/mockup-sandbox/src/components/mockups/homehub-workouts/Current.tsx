...
function WorkoutCoach({ onLogged }: { onLogged: () => void }) {
  ...
  const send = (e: React.FormEvent) => { e.preventDefault(); if (!input.trim()) return; setMessages([...messages, { id: String(Date.now()), role: "user", content: input }, { id: `${Date.now()}a`, role: "assistant", content: "Got it. I’d keep the same structure and lower the load slightly so you can move with control." }]); setInput(""); };
  return <div className="flex flex-col h-[600px] max-h-[80vh] bg-card border-2 border-border rounded-3xl overflow-hidden">
    <div className="p-4 border-b border-border bg-muted/30 flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary"><Sparkles className="w-5 h-5" /></div><div><h2 className="font-bold text-lg leading-tight">AI Workout Coach</h2><p className="text-xs text-muted-foreground">Chat, ask for routines, or adjust plans</p></div></div>
    ...
    <div className="p-4 bg-background border-t border-border"><form onSubmit={send} className="relative"><input value={input} onChange={e => setInput(e.target.value)} placeholder="E.g., adjust this for lighter dumbbells..." className="w-full bg-muted/50 border border-border rounded-full pl-4 pr-12 py-3 text-sm focus:outline-none focus:border-primary" /><button disabled={!input.trim()} className="absolute right-1.5 top-1.5 bottom-1.5 w-10 flex items-center justify-center bg-primary text-primary-foreground rounded-full disabled:opacity-50"><Send className="w-4 h-4" /></button></form></div>
  </div>;
}
...
function ExerciseLibrary() {
  ...
  return <div className="space-y-6">
    ...
    {adding && form}
    <div className="space-y-8">{groups.map(g => <div key={g.muscle} className="space-y-3"><h3 className="font-bold text-lg flex items-center gap-2 border-b border-border pb-2">{formatMuscleGroup(g.muscle)}<span className="text-xs font-medium bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{g.items.length}</span></h3><div className="grid md:grid-cols-2 gap-3">{g.items.map(ex => editing === ex.id ? <div key={ex.id} className="md:col-span-2">{form}</div> : <div key={ex.id} className="group flex items-center justify-between p-3 bg-card border border-border rounded-xl"><div><p className="font-bold text-sm">{ex.name}</p>{ex.muscleGroups.length > 1 && <p className="text-xs text-muted-foreground mt-0.5">Also: {ex.muscleGroups.slice(1).map(formatMuscleGroup).join(", ")}</p>}</div><div className="flex gap-1 opacity-0 group-hover:opacity-100"><button onClick={() => { setEditing(ex.id); setName(ex.name); }} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-primary"><Pencil className="w-4 h-4" /></button><button onClick={() => setItems(items.filter(x => x.id !== ex.id))} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-destructive"><Trash2 className="w-4 h-4" /></button></div></div>)}</div></div>)}</div>
  </div>;
}

...
import { useExtractRecipeImage, useCreateRecipe, useUpdateRecipe, getGetRecipesQueryKey } from "@workspace/api-client-react";
...
export function RecipeImport({ propertyId, onSaved }: { propertyId: string; onSaved?: (recipe: { id: string; name: string }) => Promise<void> }) {
  const [draft, setDraft] = useState<Draft | null>(null);
  const [error, setError] = useState("");
  const [isPending, setIsPending] = useState(false);
  const [createdRecipe, setCreatedRecipe] = useState<{ id: string; name: string } | null>(null);
  ...
  const choose = (picked: PickedImage[]) => {
    ...
    extract.mutate({ data: { image: picked[0].dataUrl } }, { onSuccess: value => setDraft({ ...value, ingredients: value.ingredients.map(item => ({ ...item, category: normalizeCategory(item.category) })) }), onError: () => setError("We couldn't read that recipe. A flatter, brighter photo usually helps.") });
  };
  const save = async () => {
    ...
    try {
      ...
      if (recipe) {
        ...
        recipe = { id: updated.id, name: updated.name };
        setCreatedRecipe(recipe);
      } else {
        recipe = await create.mutateAsync({
          data: { ...recipeData, propertyId },
        });
        setCreatedRecipe(recipe);
      }
      await queryClient.invalidateQueries({ queryKey: getGetRecipesQueryKey({ propertyId }) });
      if (onSaved && recipe) {
      ...
      setError(err instanceof Error ? err.message : "That recipe could not be saved. Please try again.");
    } finally {
      setIsPending(false);
    }
  };
  ...
  return (
    <section data-testid="recipe-import" className="mb-5 rounded-3xl border border-primary/15 bg-card p-4 shadow-sm">
      ...
      {draft && <fieldset disabled={isPending || create.isPending || update.isPending} data-testid="recipe-review" className="mt-4 space-y-4 border-t border-border pt-4 disabled:opacity-75">
        ...
        <div className="grid grid-cols-3 gap-2"><input data-testid="input-recipe-servings" type="number" value={draft.servings ?? ""} onChange={e => setDraft({ ...draft, servings: Number(e.target.value) || null })} placeholder="Serves" className="min-h-11 rounded-xl border border-border bg-background px-2 text-sm" /><input data-testid="input-recipe-prep" type="number" value={draft.prepMinutes ?? ""} onChange={e => setDraft({ ...draft, prepMinutes: Number(e.target.value) || null })} placeholder="Prep min" className="min-h-11 rounded-xl border border-border bg-background px-2 text-sm" /><input data-testid="input-recipe-cook" type="number" value={draft.cookMinutes ?? ""} onChange={e => setDraft({ ...draft, cookMinutes: Number(e.target.value) || null })} placeholder="Cook min" className="min-h-11 rounded-xl border border-border bg-background px-2 text-sm" /></div>
         <div className="flex gap-2"><button type="button" disabled={isPending || create.isPending || update.isPending} data-testid="button-discard-recipe-review" onClick={() => { setDraft(null); setCreatedRecipe(null); }} className="min-h-11 flex-1 rounded-xl border border-border font-semibold disabled:opacity-50">Discard</button><button type="button" data-testid="button-save-imported-recipe" disabled={isPending || create.isPending || update.isPending || !draft.name.trim()} onClick={() => void save()} className="min-h-11 flex-1 rounded-xl bg-primary font-bold text-primary-foreground disabled:opacity-50">{(isPending || create.isPending || update.isPending) ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : <><Save className="mr-2 inline h-4 w-4" />Save recipe</>}</button></div>
      </fieldset>}
    </section>
  );
}